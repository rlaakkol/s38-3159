...
\emph{teardown-ack} message and waits for timeout $T_t$ (at most ten seconds but
may be smaller) for retransmits of the \emph{teardown} message. If no retransmits 
arrive, the server may release its resources
...
to the \emph{subscribe-queryresponse} message before \emph{publish} messages are sent.
